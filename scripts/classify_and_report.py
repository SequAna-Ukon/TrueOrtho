...
def main():
    ...
    if not df_fs.empty:
        ...
        df_fs = pd.DataFrame(columns=["query", "target", "struct_score"])
    ...
    if not df_esm.empty and "target" in df_esm.columns:
        ...
        df_esm = pd.DataFrame(columns=["target", "esm2_cosine_sim"])

    # 4. Merge Metrics
    self_hit_targets = set(df_fs.loc[df_fs["query"] == df_fs["target"], "target"].astype(str)) if not df_fs.empty else set()
    df_final = pd.merge(df_esm, df_fs[["target", "struct_score"]], on="target", how="outer").fillna(0)

    if not df_final.empty:
        df_final["target"] = df_final["target"].astype(str).str.split().str[0]
        df_final = df_final.drop_duplicates(subset=["target"])
        ...
        candidates = pd.DataFrame(columns=["target", "esm2_cosine_sim", "struct_score", "orthology_confidence_score", "is_rbh", "classification"])
